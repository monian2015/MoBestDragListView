 ...
 *
 */
public class MoDragListView<T> extends ListView implements View.OnDragListener {
	...
	public MoDragListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		super.setOnDragListener(this);
	}

	public MoDragListView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public MoDragListView(Context context) {
		super(context);
	}
	...
	public boolean onDrag(View v, DragEvent event) {
		...
		case DragEvent.ACTION_DRAG_LOCATION:
			// 当前移动的item在ListView 中的position
			int position = pointToPosition((int)event.getX(), (int)event.getY());
			// 如果位置发生了改变
			if(mBeforeCurrentPosition != position){
				// 有时候得到的position 是-1(AdapterView.INVALID_POSITION),忽略掉
				if(position >=0){
...
// 如果到了两端, 判断ListView是往上滑动还是ListView往下滑动
	private void moveListViewUpOrDown(int position) {
		...
		// 能够往上的话往上
		if((position == lastPosition || position == lastPosition-1)&&firstPosition !=0){
