...
abstract class WrapperAdapter implements WrapperListAdapter,
		ItemMainLayout.OnItemSlideListenerProxy, View.OnClickListener,
	...
	public WrapperAdapter(Context context, SlideAndDragListView listView,
			ListAdapter adapter, Menu menu) {
		mContext = context;
		mListView = listView;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			itemMainLayout.setParams(mMenu.getItemHeight(),
					mMenu.getTotalBtnLength(MenuItem.DIRECTION_LEFT),
					mMenu.getTotalBtnLength(MenuItem.DIRECTION_RIGHT),
					mMenu.isWannaOver());
			createMenu(itemMainLayout);
			itemMainLayout.getItemCustomLayout().getBackGroundImage()
					.setBackgroundDrawable(mMenu.getItemBackGroundDrawable());
			...
			mAdapter.getView(position, itemMainLayout.getItemCustomLayout()
					.getCustomView(), parent);
		}
	...
	private void createMenu(ItemMainLayout itemMainLayout) {
		if (mMenu.getTotalBtnLength(MenuItem.DIRECTION_LEFT) > 0) {
			itemMainLayout.getItemLeftBackGroundLayout().getBackGroundImage()
					.setBackgroundDrawable(mMenu.getItemBackGroundDrawable());
			for (int i = 0; i < mMenu.getMenuItems(MenuItem.DIRECTION_LEFT)
					.size(); i++) {
				View v = itemMainLayout.getItemLeftBackGroundLayout()
						.addMenuItem(
								mMenu.getMenuItems(MenuItem.DIRECTION_LEFT)
										.get(i));
			...
			itemMainLayout.getItemLeftBackGroundLayout().setVisibility(
					View.GONE);
		}
		if (mMenu.getTotalBtnLength(MenuItem.DIRECTION_RIGHT) > 0) {
			itemMainLayout.getItemRightBackGroundLayout().getBackGroundImage()
					.setBackgroundDrawable(mMenu.getItemBackGroundDrawable());
			for (int i = 0; i < mMenu.getMenuItems(MenuItem.DIRECTION_RIGHT)
					.size(); i++) {
				View v = itemMainLayout.getItemRightBackGroundLayout()
						.addMenuItem(
								mMenu.getMenuItems(MenuItem.DIRECTION_RIGHT)
										.get(i));
				v.setOnClickListener(this);
				v.setClickable(false);
			...
			itemMainLayout.getItemRightBackGroundLayout().setVisibility(
					View.GONE);
		}
	}
	...
	protected void setSlideItemPosition(int position) {
		...
		ItemMainLayout itemMainLayout = (ItemMainLayout) mListView
				.getChildAt(mSlideItemPosition
						- mListView.getFirstVisiblePosition());
		for (View v : itemMainLayout.getItemLeftBackGroundLayout()
				.getBtnViews()) {
			v.setClickable(true);
		}
		for (View v : itemMainLayout.getItemRightBackGroundLayout()
				.getBtnViews()) {
			v.setClickable(true);
		}
	}
		...
		if (mSlideItemPosition != -1) {
			ItemMainLayout itemMainLayout = (ItemMainLayout) mListView
					.getChildAt(mSlideItemPosition
							- mListView.getFirstVisiblePosition());
			if (itemMainLayout != null) {
				int scrollX = -itemMainLayout.getItemCustomLayout()
						.getScrollX();
	 ...
	 */
	public void setOnAdapterSlideListenerProxy(
			OnAdapterSlideListenerProxy onAdapterSlideListenerProxy) {
	 ...
	 */
	public void setOnAdapterButtonClickListenerProxy(
			OnAdapterButtonClickListenerProxy onAdapterButtonClickListenerProxy) {
	...
	public void onClick(View v) {
		if (mOnAdapterButtonClickListenerProxy != null) {
			mOnAdapterButtonClickListenerProxy.onClick(v, mSlideItemPosition,
					(Integer) (v.getTag(TAG_LEFT) != null ? v.getTag(TAG_LEFT)
							: v.getTag(TAG_RIGHT)),
					v.getTag(TAG_LEFT) != null ? MenuItem.DIRECTION_LEFT
							: MenuItem.DIRECTION_RIGHT);
		}
	}
	...
	public abstract void onScrollStateChangedProxy(AbsListView view,
			int scrollState);

	public abstract void onScrollProxy(AbsListView view, int firstVisibleItem,
			int visibleItemCount, int totalItemCount);
